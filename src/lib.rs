...
pub fn jingle_to_sdp(jingle: &Jingle) -> Result<SessionDescription> {
  ...
  for content in &jingle.contents {
    if let Some(Description::Rtp(description)) = &content.description {
      ...
      let mut sources_by_group: Vec<(Option<&SourceGroup>, Vec<&Source>)> = vec![];
        ...
        let maybe_msid = sources
          ...
          .filter_map(|source| {
            source
              ...
              .filter_map(|param| {
                (param.name == "msid")
                  .then(|| param.value.as_ref())
                  .flatten()
              })
              .next()
          })
          .next();
        if let Some(msid) = maybe_msid {
          md = md.with_value_attribute("msid".into(), msid.clone());
        }
...
pub fn sdp_to_jingle(
  ...
  for md in &session_description.media_descriptions {
    let content = match contents.entry(md.media_name.media.as_str()) {
      ...
      Entry::Vacant(entry) => {
        ...
        description.ssrc = md.attributes.iter().filter(|attribute| attribute.key == "ssrc").next().and_then(|attribute| {
          let mut parts = attribute.value.as_ref()?.split(' ');
          Some(parts.next().unwrap().into())
        });

        for rtpmap in md.attributes.iter().filter(|attribute| attribute.key == "rtpmap") {
          if let Some(value) = &rtpmap.value {
            let mut parts = value.splitn(2, ' ');
            let id: u8 = parts.next().unwrap().parse().map_err(|_| Error::InvalidJingle)?;
            let mut parts = parts.next().ok_or(Error::InvalidJingle)?.split('/');
            let mut pt = PayloadType {
              id,
              name: Some(parts.next().unwrap().into()),
              clockrate: Some(parts.next().ok_or(Error::InvalidJingle)?.parse().map_err(|_| Error::InvalidJingle)?),
              channels: Channels(parts.next().map(|channels| channels.parse()).transpose().map_err(|_| Error::InvalidJingle)?.unwrap_or(1)),
              ptime: None,
              maxptime: None,
              parameters: vec![],
              rtcp_fbs: vec![],
            ...
            for fmtp in md.attributes.iter().filter(|attribute| attribute.key == "fmtp") {
              if let Some(value) = &fmtp.value {
                let mut parts = value.splitn(2, ' ');
                let fmtp_id: u8 = parts.next().unwrap().parse().map_err(|_| Error::InvalidJingle)?;
                if fmtp_id == id {
            ...
            for rtcp_fb in md.attributes.iter().filter(|attribute| attribute.key == "rtcp-fb") {
              if let Some(value) = &rtcp_fb.value {
                let mut parts = value.splitn(3, ' ');
                let rtcp_fb_id: u8 = parts.next().unwrap().parse().map_err(|_| Error::InvalidJingle)?;
                if rtcp_fb_id == id {
        ...
        for extmap in md.attributes.iter().filter(|attribute| attribute.key == "extmap") {
          if let Some(value) = &extmap.value {
            let mut parts = value.splitn(2, ' ');
            let id = parts.next().unwrap().parse().map_err(|_| Error::InvalidJingle)?;
        ...
        if let Some(maybe_fingerprint) = md.attribute("fingerprint") {
          transport.fingerprint = maybe_fingerprint.and_then(|fingerprint| {
            fingerprint.split_once(' ').map(|(hash, value)| Fingerprint::from_colon_separated_hex(
              match md.attribute("setup").flatten().ok_or(Error::InvalidJingle)? {
                "passive" => Setup::Passive,
                "active" => Setup::Actpass,
                "actpass" => Setup::Passive,
                _ => return Err(Error::InvalidJingle),
              },
              hash,
              value,
    ...
    if let Some(Description::Rtp(description)) = &mut content.description {
      ...
      for ssrc_group in md.attributes.iter().filter(|attribute| attribute.key == "ssrc-group") {
        if let Some(value) = &ssrc_group.value {
          let mut parts = value.split(' ');
          description.ssrc_groups.push(SourceGroup {
            semantics: parts.next().unwrap().parse().map_err(|_| Error::InvalidJingle)?,
            sources: parts.map(|id| Ok(Source {
  ...
  if let Some(group) = session_description.attribute("group") {
    let mut parts = group.splitn(2, ' ');
    jingle.group = Some(Group {
      semantics: parts.next().unwrap().parse().map_err(|_| Error::InvalidJingle)?,
      // TODO
      contents: contents.keys().map(|&name| GroupContent { name: ContentId(name.into()) }).collect(),
    });
  }
